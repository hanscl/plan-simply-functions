    ...
    try {
      ...
      if (pos_doc_before !== undefined) {
        if (
          ...
          JSON.stringify(pos_doc_before.ftes?.values) ===
            JSON.stringify(pos_doc_after.ftes?.values) &&
          pos_doc_before.pos === pos_doc_after.pos &&
          pos_doc_before.status === pos_doc_after.status
        ) {
          if (
            (pos_doc_after.status === "Hourly" &&
              pos_doc_before.rate !== undefined &&
              pos_doc_before.rate.hourly === pos_doc_after.rate.hourly) ||
            (pos_doc_after.status === "Salary" &&
              pos_doc_before.rate !== undefined &&
              pos_doc_before.rate.annual === pos_doc_after.rate.annual)
          ) {
      ...
      // Save plan id ...
      const labor_snap = await db
        .doc(
          `entities/${context_params.entity_id}/labor/${context_params.version_id}`
        )
        .get();
      if (!labor_snap.exists)
        throw new Error(
          `Could not get labor doc with version id ${context_params.version_id}. Fatal Error.`
        );
      ...
      const plan_snap = await db
        .doc(
          `entities/${context_params.entity_id}/plans/${context_params.plan_id}`
        )
        .get();
      if (!plan_snap.exists)
        throw new Error(
          `Plan document ${context_params.plan_id} does not exists for entity ${context_params.entity_id}. Exiting.`
        );
      const plan_doc = plan_snap.data() as plan_model.planDoc;

      const days_in_months = utils.getDaysInMonth(
        plan_doc.begin_year,
        plan_doc.begin_month
      );
      ...
      await labor_snap.ref
        .collection("positions")
        .doc(context_params.position_id)
        .update(pos_doc_after);

      // now update the gl account => ONLY if we have values for ACCT and DEPT
      if (
        pos_doc_after.acct !== undefined &&
        pos_doc_after.dept !== undefined
      ) {
...
function calculateWages(
  position: labor_model.positionDoc,
  ...
  for (let mnth_idx = 0; mnth_idx < 12; mnth_idx++) {
    if (position.status === "Salary" && position.rate.annual !== undefined) {
      position.wages.values[mnth_idx] =
        (days_in_months[mnth_idx] / days_in_year) *
        position.ftes.values[mnth_idx] *
        position.rate.annual;
      position.wages.total += position.wages.values[mnth_idx];
      position.wages.values[mnth_idx] = utils.finRound(
        position.wages.values[mnth_idx]
      );
      100;
    } else if (
      position.status === "Hourly" &&
      position.fte_factor !== undefined &&
      position.rate.hourly !== undefined
    ) {
      position.wages.values[mnth_idx] =
        days_in_months[mnth_idx] *
        (position.fte_factor / 52 / 7) *
        position.ftes.values[mnth_idx] *
        position.rate.hourly;
      position.wages.total += position.wages.values[mnth_idx];
      position.wages.values[mnth_idx] = utils.finRound(
        position.wages.values[mnth_idx]
      );
...
function calculateAvgFTEs(
  days_in_months: number[],
  ftes: labor_model.laborCalc
) {
  const days_in_year = days_in_months.reduce((a, b) => {
    return a + b;
  }, 0);
  ...
  for (let mnth_idx = 0; mnth_idx < 12; mnth_idx++) {
    avg_ftes +=
      ftes.values[mnth_idx] * (days_in_months[mnth_idx] / days_in_year);
  }
...
function calculateRate(position: labor_model.positionDoc) {
  ...
  if (position.status === "Hourly" && position.rate.hourly !== undefined) {
    position.rate.annual = utils.finRound(
      position.rate.hourly * position.fte_factor
    );
  } else if (
    position.status === "Salary" &&
    position.rate.annual !== undefined
  ) {
    position.rate.hourly = utils.finRound(
      position.rate.annual / position.fte_factor
    );
  }
}

async function recalcGlAccount(
  context_params: contextParams,
  updated_position: labor_model.positionDoc
) {
  // get all positions accounts for the same dept and acct
  const pos_snap = await db
    .collection(
      `entities/${context_params.entity_id}/labor/${context_params.version_id}/positions`
    )
  ...
  const acct_snap = await db
    .collection(
      `entities/${context_params.entity_id}/plans/${context_params.plan_id}/versions/${context_params.version_id}/dept`
    )
    .where("acct", "==", updated_position.acct)
    .where("dept", "==", updated_position.dept)
    .get();
