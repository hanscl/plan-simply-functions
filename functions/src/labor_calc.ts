...
export function calculateWagesEU(posData: laborModel.PositionData, ftes: number[]): laborModel.LaborCalc {
  try {
    ...
    if (posData.pay_type === "Hourly" && posData.fte_factor === undefined) throw new Error(`Cannot calculate hourly wages without an FTE factor `);
    ...
    for (let mnth_idx = 0; mnth_idx < 12; mnth_idx++) {
      ...
      } else if (posData.pay_type === "Hourly" && posData.fte_factor !== undefined && posData.rate.hourly !== undefined) {
