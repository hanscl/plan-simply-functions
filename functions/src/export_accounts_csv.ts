...
async function buildReportJson(report_params: export_model.reportRequest): Promise<export_model.acctExportCsv[]> {
  // load the entity doc
  ...
  for (const acct_id of Object.keys(acct_dict).sort()) {
    for (const dept_id of acct_dict[acct_id].depts) {
      ...
      if (fltrd_version_accts.length > 0) {
        ...
        if (fltrd_version_accts[0].acct_type !== "REV_TTL") multiplier = -1;
      }
