...
export const initVersionComparison = functions.region(config.cloudFuncLoc).https.onRequest(async (request, response) => {
  cors(request, response, async () => {
    try {
      ...
      const compParams = request.body as VersionCompParams;

      // see if comparison already exists
      const compareSnap = await db.doc(`comps/${compParams.baseVersion.versionId}/base_versions/${compParams.compareVersion.versionId}_${uid}`).get();
