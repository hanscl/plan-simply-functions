...
export const importAccountsFromCsv = functions.storage.object().onFinalize(async (object) => {
  try {
    const bucket = admin.storage().bucket(object.bucket);
    const filePath = object.name;
    const fileName = filePath?.split("/").pop();

    if (fileName === undefined || filePath === undefined) throw new Error("could not extract filename");
    ...
    const run_for_entity: RunMode = parts[1].toLowerCase() === "incl" ? RunMode.Include : RunMode.Exclude;

    if (run_for_entity === RunMode.Include) console.log(`Uploading Accounts for entities: ${entity_ids}`);
    else console.log(`Uploading Accounts for all entities except ${entity_ids}`);
    ...
    for (const entity_doc of entities_snap.docs) {
      ...
      if (run_for_entity === RunMode.Include && !entity_ids.includes(entity_doc.id)) {
        //console.log(`Skipping entity ${entity_doc.id}`);
        continue;
      }
      if (run_for_entity === RunMode.Exclude && entity_ids.includes(entity_doc.id)) {
        //console.log(`Skipping entity ${entity_doc.id}`);
        continue;
      }
      // console.log(`Evaluating entity ${entity_doc.id}`);
      // loop through the departments of this entity
      const dept_snap = await db.doc(`entities/${entity_doc.id}/entity_structure/dept`).get();
      ...
      for (const acct_row of all_acct_rows) {
        ...
        const fltrd_rollup_names = rollup_name_dicts.filter((rollup_name_entry) => rollup_name_entry.name === acct_row["Classification"]);
        if (fltrd_rollup_names.length === 0) {
          console.log(`could not find rollup name mapping for ${acct_row["Classification"]}`);
          continue;
        }
        ...
        for (const dept_id of dept_list) {
          ...
          if (acct_row[dept_id_no_cmp] !== undefined && acct_row[dept_id_no_cmp].toLowerCase() === "x") acct_dict[acct_row["GL"]].depts.push(dept_id);
      ...
      //   )}`
      // );
      // We are not overwriting the COA anymore -- so need to delete manually if it needs to be cleared out.
      await db.doc(`entities/${entity_doc.id}/entity_structure/acct`).set(acct_dict, { merge: true });
    }
  } catch (error) {
