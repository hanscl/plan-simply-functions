...
export const planViewGenerateTrigger = functions.firestore
...
export const planViewGenerateCalled = async (entityPlanVersion: {
  entityId: string;
  planId: string;
  versionId: string;
}) => {
  const context_params: contextParams = { ...entityPlanVersion, n_level_rollups: [] };
  await planViewGenerate(context_params);
};

export const planViewGenerate = async (context_params: contextParams) => {
  try {
    ...
    const versionDocSnap = await entity_snap.ref
      .collection('plans')
      .doc(context_params.planId)
      .collection('versions')
      .doc(context_params.versionId)
      .get();
    if (!versionDocSnap.exists) {
      throw new Error(`Version document not found for: ${JSON.stringify(context_params)} in [planViewGenerate]`);
    }

    const version_after = versionDocSnap.data() as plan_model.versionDoc;
    ...
    if (rollup_doc_snap.empty)
      throw new Error('Unable to find any n-level rollup accounts, which should not be happening!');
    ...
    const pnl_struct_snap = await db
      .doc(`entities/${context_params.entityId}/pnl_structures/${version_after.pnl_structure_id}`)
      .get();
    if (!pnl_struct_snap.exists)
      throw new Error('Could not find P&L Structure definition with doc id: ' + version_after.pnl_structure_id);
...
async function createPnlAggregate(
  ...
  for (const section_acct_obj of sub_accts) {
    ...
    // add to pnl_doc_ref
    pnl_doc_ref = pnl_doc_ref.concat(
      `[${section_acct_obj.account.full_account}#${convertOperationToText(section_acct_obj.operation)}]`
    );
  }

  await db
    .doc(
      `entities/${context_params.entityId}/plans/${context_params.planId}/versions/${context_params.versionId}/pnl/${pnl_doc_ref}`
    )
    .set(pnl_aggregate);
