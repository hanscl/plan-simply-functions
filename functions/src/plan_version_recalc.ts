  ...
  .onUpdate(async (snapshot, context) => {
    try {
      ...
      if (lock_status.all === true || nlevel_acct_after.is_locked === true) {
        ...
        if (
          utils.getTotalValues(nlevel_acct_after.values) !== utils.finRound(nlevel_acct_after.total) &&
          !utils.areValuesIdentical(nlevel_acct_after.values, nlevel_acct_before.values)
        ) {
